...
import { stringSkuProducts, getWarehousesByProduct, getWarehouseByProduct, getShippingMethod } from './ProductService';
...
export const createOrder = async (order, products) => {
    try {
        ...
        const goods_info = products.map(product => { return mapper(productsMapList, product) })
        ...
        if (warehouses.length > 0) {
            ...
            alert("El/Los producto(s) seleccionados no tienen stock. No se puede crear la order.")
